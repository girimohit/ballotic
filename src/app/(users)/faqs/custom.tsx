...
const queries = [
    // { id: 1, query: "SELECT * FROM voter", question: "Get all voters" },
    // { id: 2, query: "SELECT * FROM candidates", question: "Get all candidates" },
    // { id: 3, query: "SELECT * FROM wards", question: "Get all  wards" },
    {
        id: 1,
    ...
    {
        id: 6,
        query: "SELECT c.name AS candidate_name, p.party_name FROM candidates c LEFT JOIN parties p ON c.party_id = p.party_id",
        question: "Retrieve the names of candidates along with their party names "
    },
    {
        id: 7,
        query: "SELECT c.name AS candidate_name, COUNT(v.candidate_id) AS total_votes FROM candidates c LEFT JOIN votes v ON c.candidate_id = v.candidate_id GROUP BY c.candidate_id",
        question: "Retrieve the total number of votes received by each candidate"
    },
    {
        id: 8,
        query: "SELECT e.election_name, COUNT(v.vote_id) AS total_votes FROM elections e LEFT JOIN votes v ON e.election_id = v.election_id GROUP BY e.election_id",
        question: "Name of the election along with the total number of votes cast in that election"
    },
    {
        id: 9,
        query: "SELECT DISTINCT voter.username, voter.email FROM voter INNER JOIN votes ON voter.voter_id = votes.voter_id",
        question: "Username and email of voters who have cast their votes in a specific election"
    },
    {
        id: 10,
        query: "SELECT p.party_name, COUNT(c.candidate_id) AS num_candidates FROM parties p LEFT JOIN candidates c ON p.party_id = c.party_id GROUP BY p.party_id",
        question: "Names of parties along with the number of candidates"
    },
    {
        id: 11,
        query: "SELECT d.district_name, COUNT(w.ward_number) AS num_wards FROM districts d LEFT JOIN wards w ON d.district_id = w.district_id GROUP BY d.district_id",
