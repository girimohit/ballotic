...
export default function ElectionResultCard() {
    ...
    const fetchElectionData = async () => {
        try {
            const electionResponse = await fetch('/api/results')
    ...
    return (
        <>
            {election && election.map(i => (
                <div key={i.election_id} className="flex flex-col bg-gray-200 dark:bg-[#0f0f0f] rounded-3xl md:p-8 md:pb-10 p-5  w-[85%] min-h-fit md:w-[48%]  overflow-hidden my-8 border border-gray-300 dark:border-gray-600 relative drop-shadow-[0_5px_5px_rgba(255,255,255,0.25)]">
